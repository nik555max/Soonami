...
public class MainActivity extends AppCompatActivity {
    ...
    private class TsunamiAsyncTask extends AsyncTask<URL, Void, Event> {
        ...
        private String makeHttpRequest(URL url) throws IOException {
            String jsonResponse = "";
            //if url is null then we are not goint to send GET request
            if(url == null){
                return jsonResponse;
            }
            ...
            try {
                ...
                if (urlConnection.getResponseCode() == 200){
                    //this tow methods will only be executed if the status code is 200 : OK
                    inputStream = urlConnection.getInputStream();
                    jsonResponse = readFromStream(inputStream);
                }else{
                    Log.e(LOG_TAG, "Status code returned is "+urlConnection.getResponseCode());
                }

            } catch (IOException e) {
                // TODO: Handle the exception
                Log.e(LOG_TAG, "IOException is thrown"+e.getMessage(), e);
            } finally {
